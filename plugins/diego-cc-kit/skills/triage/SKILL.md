...
5. Detect the mode. At the repo root run `jq -r '.outputStyle // empty' .claude/settings.local.json 2>/dev/null`. Output `diego-cc-kit:aprender` (the plugin-namespaced style name; a bare `aprender` loads no style) → **learn mode**: the plan carries a `## Tajada [human]` section. Anything else (empty, `Default`, missing file) → **deliver mode**: omit that section. Everything else in the contract applies to both modes.
