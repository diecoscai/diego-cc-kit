---
name: drill
description: Spaced-recall drill over docs/agent/til.md — asks five concepts from recent work, records hits and misses. User-invoked as /diego-cc-kit:drill; run weekly in learn-mode repos.
argument-hint: "[path to til.md]"
...
5. Append ` [✓ YYYY-MM-DD]` or ` [✗ YYYY-MM-DD]` with today's date (`date +%F`) to each drilled line. Commit only that file: `git add docs/agent/til.md && git commit -m "docs: drill YYYY-MM-DD"`.
6. Close with the score (`3/5`) and, if no TIL line whose own date (the first field) falls in the last 30 days contains the word `walkthrough`, one extra line in the user's language, e.g.: "Toca un linear walkthrough: pedí uno del módulo que más creció este mes, leelo entero y dejá una línea `walkthrough` en til.md."
