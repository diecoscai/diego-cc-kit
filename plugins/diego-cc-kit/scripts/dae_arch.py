#!/usr/bin/env python3
# Vendored from swingerman/disciplined-agentic-engineering (MIT), 2026-07-27,
# upstream path: engineer/scripts/dae_arch.py. Local modifications are marked
# LOCAL: below — a `--manifest PATH` flag defaulting to `.claude/arch.yml`,
# since this kit has no `.engineer/manifest.yml` onboarding flow. Everything
# else is unmodified upstream. dae_resolve.py is a required companion
# module, vendored alongside it unmodified.
"""dae_arch.py — charter architecture fitness checker.

Reads the `architecture:` section of a manifest YAML and checks the project
against it: dependency layering, cycles, forbidden patterns, file naming,
and file size. Reports violations and exits non-zero if any are found — it
is a gate, like a test.
...
Usage:
  ...
  dae_arch.py --manifest PATH [...]  LOCAL: manifest path (default .claude/arch.yml)
...
# LOCAL: upstream hardcodes `.engineer/manifest.yml`; this kit uses its own
# convention and a CLI-configurable path (see --manifest / _resolve below).
DEFAULT_MANIFEST_REL_PATH = os.path.join(".claude", "arch.yml")
...
def _find_manifest(start_dir, rel_path):
    """LOCAL: walk up from start_dir for `rel_path`.

    Mirrors dae_resolve.find_methodology_root but with a configurable
    relative manifest path instead of the hardcoded `.engineer/manifest.yml`.
    Returns (root, manifest_path) or (None, None).
    """
...
def _resolve(start_dir, rel_path):
    """LOCAL: dae_resolve.resolve(), but for a configurable manifest path.

    Reuses dae_resolve's (unmodified) parser/validator — only the file
    lookup differs from upstream's hardcoded `.engineer/manifest.yml`.
    """
    root, manifest_path = _find_manifest(start_dir, rel_path)
    if root is None:
        return None
    ...
    try:
        ...
        parse_error = None
    except dae_resolve.ManifestError as exc:
        manifest, parse_error = {}, str(exc)
    if parse_error:
        errors, warnings = ["manifest parse error: " + parse_error], []
    else:
        errors, warnings = dae_resolve.validate_manifest(manifest)
    return {
...
def audit(start_dir, full, manifest_rel_path=DEFAULT_MANIFEST_REL_PATH):
    ...
    """
    result = _resolve(start_dir, manifest_rel_path)  # LOCAL: was dae_resolve.resolve(start_dir)
    if result is None:
...
def main(argv):
    ...
    if "--format" in args:
        ...
        del args[i:i + 2]
    manifest_rel_path = DEFAULT_MANIFEST_REL_PATH
    if "--manifest" in args:  # LOCAL: configurable manifest path
        i = args.index("--manifest")
        manifest_rel_path = args[i + 1] if i + 1 < len(args) else manifest_rel_path
        del args[i:i + 2]
    full = False
    if "--full" in args:
        full = True
    ...
    if len(args) > 1:
        sys.stderr.write(
            "usage: dae_arch.py [--full] [--format json] [--manifest PATH] [START_DIR]\n")
        return 3
    start_dir = args[0] if args else os.getcwd()

    root, rules, violations = audit(start_dir, full, manifest_rel_path)
    if root is None:
        sys.stderr.write(
            "no %s found walking up from %s\n"
            % (manifest_rel_path, os.path.abspath(start_dir)))
        return 2
    if rules is None:
