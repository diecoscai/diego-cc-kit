...
hours="${1:-24}"
# SUBAGENT_DIAG_PROJ: override para tests (evita tener que redirigir HOME real).
proj="${SUBAGENT_DIAG_PROJ:-$HOME/.claude/projects/$(pwd | sed 's|[/._]|-|g')}"
...
while IFS= read -r f; do
  ...
  last_ts="$(tail -1 "$f" | jq -r '.timestamp // "?"' 2>/dev/null)" || true
