...
- **output-styles/** — `aprender`: set `"outputStyle": "diego-cc-kit:aprender"` (plugin styles are namespaced) in a repo's `.claude/settings.local.json` and the main conversation debugs with hints (max three) before fixing, leaves one `TODO(human)` slice per task, and names the pattern it uses. The same field is what `triage`/`implement` read to enter learn mode; leave it unset in repos where you only deliver.
